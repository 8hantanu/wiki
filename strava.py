...
STRAVA_CLIENT_ID = "201771"
STRAVA_CLIENT_SECRET = "2fb044435f1a4bd631c5e124209e367561d608d9"
STRAVA_REFRESH_TOKEN = "02b143ea32de63cb453927a18df007f01ae71c2c"
...
print("\n   Analyzing activity sources...")
for activity in all_activities[:5]:  # Check first 5
    if activity['type'] in ['Run', 'VirtualRun']:
        print(f"   Sample: {activity['name']}")
        print(f"     - device_name: {activity.get('device_name', 'N/A')}")
        print(f"     - upload_id: {activity.get('upload_id', 'N/A')}")
        print(f"     - external_id: {activity.get('external_id', 'N/A')}")

total_distance_meters = 0
run_count = 0
seen_timestamps = {}  # Track activities by start time to avoid duplicates

print("\n   Counting unique Garmin runs...")
...
for activity in all_activities:
    if activity['type'] in ['Run', 'VirtualRun']:
        ...
        print(f"   ✓ {date}: {name} ({distance_km:.2f} km) [{device_name}]")
